...
const USERS_BY_USERNAME_API_URL = 'https://api.twitter.com/2/users/by';

/**
 * @throws Exception
 */
function get_users_by_username(array $headers, array $params = []): array
{
    $url = USERS_BY_USERNAME_API_URL.'?'.http_build_query($params);
    ...
    if ($status_code != 200) {
        ...
        throw new Exception("Unable to get users: ".$errors, $status_code);
    }
...
try {
    ...
    $users = get_users_by_username($headers, $params);
} catch (Exception $e) {
    print $e->getMessage();
}

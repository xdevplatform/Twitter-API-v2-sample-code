...
const SPACES_API_URL = 'https://api.twitter.com/2/spaces';

/**
 * @throws Exception
 */
function get_spaces(array $headers, array $params = []): array
{
    $url = SPACES_API_URL.'?'.http_build_query($params);
    ...
    if ($status_code != 200) {
        ...
        throw new Exception("Unable to get spaces: ".$errors, $status_code);
    }
...
try {
    ...
    $params = [
        'ids' => '1dRKZMrWAkDxB,1ynJOadbnawKR',
        'space.fields' => 'title,created_at'
    ];

    $spaces = get_spaces($headers, $params);
} catch (Exception $e) {
    print $e->getMessage();
}

...
const SEARCH_RECENT_TWEETS_API_URL = 'https://api.twitter.com/2/tweets/search/recent';

/**
 * @throws Exception
 */
function get_recent_tweets(array $headers, array $params = []): array
{
    $url = SEARCH_RECENT_TWEETS_API_URL.'?'.http_build_query($params);
...
try {
    ...
    $tweets = get_recent_tweets($headers, $params);
} catch (Exception $e) {
    print $e->getMessage();
}
